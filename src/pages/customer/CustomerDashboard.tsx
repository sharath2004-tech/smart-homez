          ...
          >
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-foreground">
                {locationLoading
                  ? t('customer.dashboard.checkingRegion')
                  : serviceableStatus === 'unavailable'
                  ? t('customer.dashboard.areaNotServiceable')
                  : t('customer.dashboard.setLocationToSee')}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {locationLoading
                  ? t('customer.dashboard.tracingLocation')
                  : serviceableStatus === 'unavailable'
                  ? (serviceabilityMessage || t('customer.dashboard.bookElsewhere'))
                  : (locationError || t('customer.dashboard.chooseLocationDesc'))}
              </p>
              {nearestServiceArea?.name && serviceableStatus === 'unavailable' && (
                <p className="text-xs text-amber-700 mt-2">
                  {typeof nearestServiceArea.distance === 'number'
                    ? t('customer.dashboard.nearestServiceAreaWithDistance', { name: nearestServiceArea.name, distance: nearestServiceArea.distance.toFixed(1) })
                    : t('customer.dashboard.nearestServiceArea', { name: nearestServiceArea.name })}
                </p>
              )}
            </div>
            <div className="flex shrink-0 flex-col gap-2">
              ...
              >
                {serviceableStatus === 'unavailable' ? t('customer.dashboard.chooseLocation') : t('customer.dashboard.setLocation')}
              </button>
              {serviceableStatus === 'unavailable' && selectedLocation && deepCleaningRequestServiceId && (
                ...
                >
                  {requestingDeepCleaning ? t('customer.dashboard.requesting') : t('customer.dashboard.requestDeepCleaning')}
                </button>
              )}
              {locationError && (
                <button
                ...
                >
                  <RefreshCw className="h-3 w-3" /> {t('customer.dashboard.retry')}
                </button>
              )}
            </div>
          </motion.div>
            ...
            >
              {upcomingBookings.map((b, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-foreground text-sm">{b.service?.name ?? (b.bookingType === 'deep-cleaning-cart' ? t('customer.dashboard.deepCleaning') : t('common.booking'))}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">
