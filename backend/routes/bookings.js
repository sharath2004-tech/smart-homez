...
// @access  Customer
// ─────────────────────────────────────────────────────────────────────────────
router.get('/customer/my-subscriptions', authenticate, authorize('customer'), async (req, res) => {
  try {
    ...
    const result = await Promise.all(validParentBookings.map(async (b) => {
      const childSessions = await Booking.find({
        ...
        .select('status bookingDate startTime endTime actualStartTime actualEndTime actualDurationMinutes scheduledDurationMinutes overtimeMinutes overtimeCharges worker')
        .populate('worker', 'name')
        .sort({ bookingDate: 1 })
        .lean();
      ...
      return {
        ...
        autoRenewal: b.subscription?.autoRenewal,
        allowPause: b.subscription?.allowPause,
        pauseRequestStatus: b.subscription?.pauseRequestStatus,
        pauseRequestedAt: b.subscription?.pauseRequestedAt,
        pauseRequestStartDate: b.subscription?.pauseRequestStartDate,
        pauseRequestEndDate: b.subscription?.pauseRequestEndDate,
        pauseReviewNote: b.subscription?.pauseReviewNote,
        ...
        selectedDays: b.recurringSchedule?.selectedDays,
        startDate: b.recurringSchedule?.startDate || b.subscription?.subscriptionStartDate,
        endDate: b.recurringSchedule?.endDate || b.subscription?.subscriptionEndDate,
        totalAmount: b.totalAmount,
